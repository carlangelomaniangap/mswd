...
return new class extends Migration
{
    ...
    {
        Schema::create('sp_spouse_with_pmi_reqs', function (Blueprint $table) {
            $table->id();
            ...
            $table->enum('marriage_cert_or_affidavit_of_cohabitation', ['Complete', 'Incomplete', 'Renewal', 'Denied'])->default('Incomplete');
            $table->date('marriage_cert_or_affidavit_of_cohabitation_expires_at')->nullable();
            $table->date('marriage_cert_or_affidavit_of_cohabitation_updated_at')->nullable();
            $table->enum('med_record_med_abstract_or_cert_of_confinement', ['Complete', 'Incomplete', 'Renewal', 'Denied'])->default('Incomplete');
            $table->date('med_record_med_abstract_or_cert_of_confinement_expires_at')->nullable();
            $table->date('med_record_med_abstract_or_cert_of_confinement_updated_at')->nullable();
            $table->enum('a4_solo_parent_not_cohabiting', ['Complete', 'Incomplete', 'Renewal', 'Denied'])->default('Incomplete');
            $table->date('a4_solo_parent_not_cohabiting_expires_at')->nullable();
            $table->date('a4_solo_parent_not_cohabiting_updated_at')->nullable();
    ...
    {
        Schema::dropIfExists('sp_spouse_with_pmi_reqs');
    }
};
