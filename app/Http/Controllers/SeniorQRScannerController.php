...
class SeniorQRScannerController extends Controller
{
    public function index(Request $request)
    {
        return view('pages.senior_qrcode_scanner', [ 
    ...
    public function scan($id) 
    {
        ...
        // Split the QR code value (e.g. "SC-001") into two parts: SC as type and 001 as id
        // $type = record type (SC)
        ...
        if ($type === 'SC') {
            $record = SeniorCitizenRecord::with('seniorRequirement')->find($id);
        ...
        $family_members = $record->seniorfamilyMember->map(function($member) {
            return [
                'sc_record_id' => $member->sc_record_id,
