    ...
    <div class="p-6 overflow-y-auto bg-white rounded-md shadow-md dark:bg-dark-eval-1">
        ...
        <x-modal name="view" maxWidth="2xl">
            <div x-data="{ tab: 'personal_details' }" x-show="true"  @open-modal.window="if ($event.detail === 'view') tab = 'personal_details'" class="max-h-full flex flex-col">
                ...
                <div class="bg-white dark:bg-dark-eval-1 flex items-center justify-between p-4 text-sm shadow-md">
                                    ...
                                    <div class="flex flex-col">
                                        ...
                                        <div class="p-4">
                                            <form id="addFamilyMemberForm" method="POST">
                                                @csrf
                                                <input type="hidden" name="pwd_record_id" id="pwd_record_id">
                                                <div class="space-y-4">
                                                    <div>
                                                        <x-form.label
...
{{-- Display Family Composition Script and Add Family Member --}}
<script>
    $(document).on('click', '[data-id]', function () {
        const id = $(this).data('id');
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                $('#family_member').DataTable({
                    ...
                    ajax: {
                        url: `/admin/pwd/${id}/family-member`,
                        dataSrc: 'data'
                    },
                    ordering: false,
                    columns: [
...
<script>
    $(document).ready(function () {
        $('#addFamilyMemberForm').on('submit', function (e) {
            ...
            $.ajax({
                url: `/admin/pwd/store/family-member`,
                method: 'POST',
                data: formData,
                processData: false,
                contentType: false,
